let data = [
    {value: '接下来是听妈妈的话鸭！', time:5, color: 'red', speed: 1, fontSize:22},
...
// 创建CanvasBarrage类
class CanvasBarrage {
    // constructor是一种用于创建初和始化class创建的对象的特殊方法
    constructor(canvas, video, opts = {}){ //opts={}如果没有传值，也不会报错

        // 如果canvase和video都没传，那就直接return掉
...
class Barrage {
    constructor(obj, ctx) {
        ...
        // 把obj 和 ctx都挂载到this上，方便获取
        this.obj = obj
        this.context = ctx
    }
    // 初始化弹幕
    init() {
        // 如果数据里没有涉及到下面4种参数， 那就直接取默认参数
